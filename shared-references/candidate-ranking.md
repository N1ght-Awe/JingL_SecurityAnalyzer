...
P0/P1 必须逐条深审；尚未完成的项标“待审查”并说明原因。P2/P3 根据影响、不确定性和用户重点选择深审或验证。未选择项仍在结构化结果中。明确排除由 `status=误报` 表达，不降到 P3 代替排除。

分级后只对高价值目标选择模拟：目标明确、关键代码可用、对照可设计、结果会影响判断或修复，且执行环境符合用户授权。高危但环境缺失的项仍保留高危初评和阻塞原因。
