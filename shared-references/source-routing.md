...
建立范围时分别记录执行平台可访问的定义/引用/实现/调用层次能力、索引就绪状态及模块覆盖，再记录AST工具和文本搜索能力。IDE可见但模型不可调用的能力记为不可用。按 call-chain-tracing.md 配合使用；原始查询/读取按 execution-supervision.md 留证。AST 不可用与源码缺失是不同限制，不能相互替代；搜索范围与忽略文件须和资产清单对账。
