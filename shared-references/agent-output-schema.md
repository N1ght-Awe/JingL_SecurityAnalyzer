...
候选需有稳定 ID、仓库/版本、源位置、类型、初评及依据、已读实现、未读缺口。证据使用文件引用及摘要，不复制大段日志。防护知识可共享已审实现与指纹，但新调用点的适用性由当前审查者提供证据；合并前核验 control_applications 引用与版本。分工数量不证明覆盖度；主审核对范围并集、缺口、重复和跨仓边是否接上。
