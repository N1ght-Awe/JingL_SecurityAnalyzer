...
$requiredRefs = @(
  ...
  'protection-audit-methodology.md', 'control-knowledge.md'
)
...
foreach ($versionField in @('skill_version', 'rules_version')) {
  if ($text -notmatch "$($versionField): 2\.1\.0") { throw "Stale version: $versionField" }
}
foreach ($path in @('scripts/validate-report.py', 'tests/test_report_contract.py', 'tests/test_report_extensions.py')) {
