...
| `shared-references/` | 22 份范围、证明、验证、防护复用和报告引用，按需读取 |
...
新报告用 schema/skill/rules 2.1.0；历史 schema 2.0.0（规则2.0.0/2.0.1）继续兼容，不能只改版本号伪装获得新覆盖检查。防护知识是有证据的 JSON 审查记录，由执行模型读取、比对和复用，不是自动训练模型或无需人工证据的缓存引擎。
