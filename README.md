...
新报告使用 schema 2.3.0、skill/rules 2.3.1，要求原始候选对账、逐实例监督和源码侧初始复核，保留轻量threat_model；历史2.2监督及2.1覆盖/复用契约继续兼容；历史 schema 2.0.0（规则2.0.0/2.0.1）继续兼容，不能只改版本号伪装获得新覆盖检查。防护知识是有证据的 JSON 审查记录，由执行模型读取、比对和复用，不是自动训练模型或无需人工证据的缓存引擎。
