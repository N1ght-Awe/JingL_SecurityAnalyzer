...
7. API响应URL绑定href等位置时，追实际URL构造及可影响部分；“系统生成”或“后端不会返回恶意协议”不是反证。确认协议受固定构造/有效约束限制后可排除对应候选；来源未查清保留缺口，不批量降级同模式实例。
8. URL校验区分子串命中、协议检查和整串格式检查；仅含http子串不足以约束实际协议，缺少末尾锚点本身也不必然是漏洞。核验解析/规范化、允许协议与最终消费语义；是否支持相对URL、blob等由业务明确，不能给通用白名单任意扩容。
...
URL优先使用与消费端语义一致的解析和协议允许集合，必要时限制目标；HTML转义不能代替协议限制。HTML文本、属性、URL和内联脚本采用各自安全输出方式，不用单个通用转义函数覆盖全部上下文。
