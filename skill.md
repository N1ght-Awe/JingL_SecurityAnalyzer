...
用户说“jingl 扫描 <代码仓或汇总根目录>”时使用。仅分析用户指定范围；正式覆盖 Java / Spring Boot / Jalor，关联前端或其他语言时明确实际覆盖能力。Skill 与规则版本为 2.0.1，报告契约仍为 2.0.0。

## 核心执行约束

- 先扫描、去重和初步分级，再聚焦高价值目标进行深审与必要验证；不为每个候选全面展开模拟。验证范围以证明该漏洞目标所需的最小真实链路为准。
