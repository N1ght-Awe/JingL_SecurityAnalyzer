...
用户说“jingl 扫描 <代码仓或汇总根目录>”时使用。仅分析用户指定范围；正式覆盖 Java / Spring Boot / Jalor，关联前端或其他语言时明确实际覆盖能力。Skill 与规则版本为 2.2.1，新报告契约为 2.2.0，校验器继续接受历史 2.0.0 / 2.1.0 契约。
...
2. **候选发现。** 按索引发现候选，再优先用可用语义导航读取定义和调用关系，AST与文本搜索补充定位，再读命中上下文；结构、配置、数据流检查补充关键词。搜索截断必须分片重试或明确未覆盖。保存位置和摘要，不把整仓内容送入每次推理。
3. **去重与初评。** 同根因、同路径防护合并，保留受影响入口；分别记录初评 P0–P3 和证据缺口。筛选出的 P0/P1/P2 均进入深审与深度验证，按严重度安排顺序；P3 保留清单，不默认进入深度验证。高后果且不确定的项不得仅因证据少而压入低级。
4. **类型深审与全局关联。** 按 call-chain-tracing 与 execution-supervision 保存链路工作清单和实际读取回执。读取必要子 Skill，追踪真实实现、安全拦截器、配置、依赖和跨仓边界；合并新的等价路径，避免重复测试。补充事实可随时调整分级。未审项标“待审查”，不得消失。
...
7. **审定与报告。** 按 execution-supervision 独立复核原始结果，重新寻找遗漏防护；默认最多两轮定向补查，无进展停止，未解决项保留缺口。按 proof-schema 审查证据和可宣称影响，生成完整结构化结果；按 report-schema 固定顺序输出一份主报告及四个原有视图，威胁概览置于问题清单之前。默认运行 render-report.py 校验并生成 Markdown，不让排版改写结论；统计遗漏、未验证原因和实际耗时/token。确认问题后给出针对真实实现的修复方向，不自动修改被扫描业务代码。
