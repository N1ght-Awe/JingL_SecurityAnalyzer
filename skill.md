...
用户说“jingl 扫描 <代码仓或汇总根目录>”时使用。仅分析用户指定范围；正式覆盖 Java / Spring Boot / Jalor，关联前端或其他语言时明确实际覆盖能力。Skill 与规则版本为 2.3.2，新报告契约为 2.3.0，校验器继续接受历史 2.0 / 2.1 / 2.2 / 2.3 契约，不替历史证据升级。

## 核心执行约束

- 可控性按实际信任边界判断，客户端回传不因源自服务器而可信。危险模式、路径可达、漏洞可利用是不同证明层次；确认前主动寻找同路径反证。源码足够时保留源码确认，真实目标验证通过才给确认；mock 命中和断言数量不能扩大证明范围。
- 完成资产枚举再给覆盖结论。必需资产缺失/未审允许部分报告，禁止声称完整覆盖或给未覆盖范围下阴性结论。
- 已核验防护实现可在本轮即时复用；每条新路径仍检查实际调用、适用参数、使用校验结果、无绕过与后续变换。应用的实现/依赖/配置指纹须匹配本轮读取快照，缺失或变化保留 unresolved，不能缓存“接口永久安全”。
...
- 必须区分源码判断、局部模拟和真实目标行为。PASSED 须按 poc-execution 绑定本轮、实际列出的调用实例与已审源码；模型解释、复制的漏洞逻辑、模拟返回值都不能充当目标执行证据。
...
3. **去重与初评。** 同一实例重复命中沿用ID；同根因的不同调用实例关联但分别保留结论，候选数按账本实例计而非根因分组数；分别记录初评 P0–P3 和证据缺口。筛选出的 P0/P1/P2 均进入深审与深度验证，按严重度安排顺序；P3 保留清单，不默认进入深度验证。高后果且不确定的项不得仅因证据少而压入低级。
